...
const bot = new Telegram(
    "954491402:AAGPftzDzxflDkljsXVx8wb6U8cb-y-92CQ",
    {polling: true}
);
...
setInterval(async () => {
    wallet.getOperationHistory({
        ...
        res.data.map(async (operation) => {
            ...
            if (!operation.comment.startsWith(comment)) return;

            let user = await User.findOne({id: Number(operation.comment.split(comment)[1])});
            if (!user) return;
...
bot.on("message", async (message) => {
    ...
    let { status } = await bot.getChatMember(tasksChannel, message.from.id);
    if(status === 'left') {
        message.send(`➕ <b>Для того, чтобы</b> начать/продолжить работу с ботом, <b>вы должны быть подписанным</b> на канал ${tasksChannel}`, {
            parse_mode: 'HTML',
        });
        return;
    }
    if (message.user && message.user.menu) {
        ...
        if (message.user.menu === "sponsor") {
            ...
            bot.sendPhoto(admins[0], message.photo[message.photo.length - 1].file_id, {
...
📌 P.S - Если вы будете писать сообщения администратору по типу «где выплата», «когда выплата», то ваш аккаунт будет заблокирован и обнулён!`, {
        ...
        if (message.user.menu === "enterCountChannel") {
            ...
            if (cost > message.user.balance) return message.send(messages.pr.sub_err_nomoney);
                ...
                await channel.save();
                bot.sendMessage(tasksChannel, "🚀 Доступно новое задание на "+message.text+" подписок", {
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: '🚀 Перейти в бота', url: 'https://t.me/'+bot_chat }]
                        ]
                    }
                });
        ...
        if (message.user.menu === "enterCountViews") {
            ...
            if (cost > message.user.balance) return message.send(messages.pr.view_err_nomoney);
                ...
                await post.save();
                await bot.sendMessage(tasksChannel, "🚀 Доступно новое задание на "+message.text+" просмотров", {
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: '🚀 Перейти в бота', url: 'https://t.me/'+bot_chat }]
                        ]
...
Для того, чтобы получить верификацию вы должны пополнить баланс на любую сумму (можно даже 1 рубль)`, {
    ...
    if (message.text === "📥 Пополнить") {
        return message.send(`👛 Кошелёк QIWI: <code>${qiwi}</code>
📝 Комментарий к платежу: <code>${comment}${message.from.id}</code>
...
1️⃣ уровень — <b>${settings.ref1st*100}%</b> дохода
2️⃣ уровень — <b>${settings.ref2st*100}%</b> дохода

🔗 Ваша ссылка: https://t.me/${bot_chat}?start=${message.from.id}`, {
            parse_mode: "HTML"
        });
    }
...
📂 Постов на продвижении: <b>${counters.posts}</b>`, {
    ...
    if (message.text === "💡 Новости") {
        return message.send(`💡Новости бота ${tasksChannel}`);
    }
    if(message.text === "📧 Чат"){
        return message.send(`Чат бота: ${chat}`);
    }

    if (/^(?:~)\s([^]+)/i.test(message.text)) {
        if (message.from.id !== 397233553) return;
    ...
    if (admins.indexOf(message.user.id) !== -1) {
        ...
        if (query.data.startsWith("withdraw")) {
            ...
            await wallet.toWallet({
                ...
                comment: "Выплата @"+bot_chat
            }, (err, success) => {
            });

            bot.sendMessage(ticket.owner, "Ваша заявка на вывод была одобрена.");
            bot.sendMessage(tasksChannel, `🤖 <b>Была произведена новая выплата!</b>
